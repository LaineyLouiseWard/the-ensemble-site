...
description: |
  From Alzheimer's ChatGPT to agentic terminal assistants — a first look at Claude Code and what it actually offers for research workflows.
pubDate: 'Mar 7 2026'
authors: ['lainey']
track: 'phd-tips'
featured: true
...
But for someone whose research is AI-based, it felt archaic.

One day, I wondered if I could create my own local ChatGPT. Then I discovered I didn't have to. I found the deep end of the LLM spectrum — Claude Code.
...
Claude Code differs in that it is an agentic assistant that runs in your terminal. It is like a local ChatGPT that can see inside your project repository and gather context itself. Not just the snippet you pasted. The actual structure. The dependencies. The READMEs.

I have been testing it over the last week or two with things to report back on. I tested both the free tier and the paid subscriptions — Pro ($17) and Max 5x ($100). I found Pro not to be enough for daily usage, and Max 5x has not run out of tokens once for me, which is a relief given the price. First though, if you're considering paying for it, understand the currency.

## Tokens

There are two limits to keep in mind. Each chat has a context window of roughly 200k tokens. When it fills up, you can compact the conversation into a summary and continue, but it is not infinite.

There are also session and usage limits. Depending on your plan, there are rolling time-based limits and weekly caps across chats. Claude Code and ChatGPT both operate within large but finite context windows. The difference with Claude Code is that you have more ways to conserve tokens while still giving it access to your full repo context.
...
**Skills** are written recipes pinned to the wall. They are markdown files that define repeatable workflows — a code review, a documentation check, paper formatting rules. Instead of restating your standards every session, you encode them once and reuse them. There is even a meta-skill that writes skills for you. I used it to create one that tells Claude how to read my repo, check the README and CLAUDE.md files, and keep conventions consistent when I run forecast verification.
...
Research is all about repositories, analysis scripts, reproducibility, rerunning pipelines, and documenting everything properly. On paper, it feels like a perfect fit — so why aren't more researchers using it?
