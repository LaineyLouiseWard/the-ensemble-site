 ...
 *   2. Neither configured → fallback (returns 0, logs warning)
 */
...
class FallbackStore implements ViewsStore {
	...
	async increment(_slug: string): Promise<number> {
		if (!this.warned) {
			console.warn(
				'[views] No storage configured. Set UPSTASH_REDIS_REST_URL + UPSTASH_REDIS_REST_TOKEN to enable view counting. Returning 0.'
			)
			this.warned = true
		}
		return 0
...
export function getViewsStore(): ViewsStore {
